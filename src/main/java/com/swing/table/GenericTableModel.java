...
import com.swing.table.i18n.I18NUtil;
...
public class GenericTableModel<E> extends AbstractTableModel {
    ...
    public GenericTableModel(E e, Locale locale) {
        super();
        this.e = e;
        this.locale = locale;
        setAllColumns();
	...
	private void setCellRenderers() {
	    for (Field field:fields) {
	        ColumnFormat format = field.getAnnotation(ColumnFormat.class);
	        if(format != null){
	...
	public String getColumnName(int column) {
		...
		if (colName != null) {
		    String localeCode = colName.locale();
		    Locale currentLocale = locale;
		    if(localeCode != null && !localeCode.equals("") 
		            && !locale.toString().equals(localeCode)){
		        ...
		        currentLocale = new Locale(language, cntry); 
		    }
		    String i18nKey = colName.i18nKey();
		    columnName = I18NUtil.getInstance(currentLocale).getString(i18nKey);
		} else {
	...
	public Class getColumnClass(int c) {
		return getFields().get(c).getClass();
	}
